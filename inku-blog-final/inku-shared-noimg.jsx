...
// deterministic tint pick per string seed (for placeholders)
function seedHue(s, offset = 0) {
  let h = 0;
  for (let i = 0; i < s.length; i++) h = (h * 31 + s.charCodeAt(i)) >>> 0;
  return (h + offset) % 360;
}
const TINT_PALETTE = ['#D02886', '#9966CC', '#54218E', '#FF5FB3', '#1A1A2E'];
function pickTint(seed) {
  let h = 0;
  for (let i = 0; i < seed.length; i++) h = (h * 31 + seed.charCodeAt(i)) >>> 0;
  return TINT_PALETTE[h % TINT_PALETTE.length];
}

// Cover placeholder — SOLID color
function CoverArt({ seed = 'inku', label, ratio = '16/9', children, style = {} }) {
  const bg = pickTint(seed);
  return (
    <div style={{
      position: 'relative',
      width: '100%',
...
function StarRating({ score, size = 14, color = INKU.magenta }) {
  ...
  return (
    <span style={{ display: 'inline-flex', gap: 2, alignItems: 'center', color }}>
      ...
      <span style={{ fontFamily: "-apple-system, BlinkMacSystemFont, 'Segoe UI', Helvetica, Arial, sans-serif", fontWeight: 700, fontSize: size, color: INKU.ink, marginLeft: 4 }}>{score.toFixed(1)}</span>
    </span>
  );
}
...
function Tag({ children, variant = 'default', style = {} }) {
  ...
  return (
    <span style={{
      ...
      fontFamily: "-apple-system, BlinkMacSystemFont, 'Segoe UI', Helvetica, Arial, sans-serif", fontWeight: 700, fontSize: 12,
...
// Logo (wordmark) — pure CSS, no image
function Logo({ height = 28, onClick }) {
  return (
    <div onClick={onClick} style={{
      ...
      <span style={{
        width: height * 0.95, height: height * 0.95, borderRadius: 6,
        background: INKU.magenta, display: 'inline-block',
      }} />
      <span style={{
        fontFamily: "-apple-system, BlinkMacSystemFont, 'Segoe UI', Helvetica, Arial, sans-serif",
        fontWeight: 800,
          ...
          {items.map(it => {
            ...
            return (
              <button key={it.id} onClick={() => onNav(it.id)} style={{
                fontFamily: "-apple-system, BlinkMacSystemFont, 'Segoe UI', Helvetica, Arial, sans-serif", fontWeight: 600, fontSize: 14,
        ...
        <div style={{ position: 'relative', width: 240 }}>
          <input
            ...
            style={{
              ...
              fontFamily: "-apple-system, BlinkMacSystemFont, 'Segoe UI', Helvetica, Arial, sans-serif", fontSize: 13,
    ...
    return (
      <form onSubmit={submit} style={{
        ...
        <input value={email} onChange={(e)=>setEmail(e.target.value)} type="email" required placeholder="ton@email.com"
          style={{ flex: 1, border: 'none', outline: 'none', padding: '12px 14px', fontFamily: "-apple-system, BlinkMacSystemFont, 'Segoe UI', Helvetica, Arial, sans-serif", fontSize: 14, background: 'transparent', color: INKU.ink }} />
        <button type="submit" style={{
          background: INKU.magenta, color: '#fff', border: 'none',
          padding: '0 20px', fontFamily: "-apple-system, BlinkMacSystemFont, 'Segoe UI', Helvetica, Arial, sans-serif", fontWeight: 700, fontSize: 13,
    ...
    <section style={{
      ...
      <div style={{ position: 'relative', display: 'grid', gridTemplateColumns: '1.2fr 1fr', gap: 48, alignItems: 'center' }}>
        <div>
          ...
          <p style={{ fontFamily: "-apple-system, BlinkMacSystemFont, 'Segoe UI', Helvetica, Arial, sans-serif", fontSize: 16, lineHeight: 1.55, color: 'rgba(255,255,255,0.85)', maxWidth: 460, margin: '0 0 24px' }}>
          ...
          <form onSubmit={submit} style={{
            ...
            <input value={email} onChange={(e)=>setEmail(e.target.value)} type="email" required placeholder="ton@email.com"
              style={{ flex: 1, border: 'none', outline: 'none', background: 'transparent', color: '#fff',
                padding: '10px 12px', fontFamily: "-apple-system, BlinkMacSystemFont, 'Segoe UI', Helvetica, Arial, sans-serif", fontSize: 15 }} />
            <button type="submit" style={{
              background: '#fff', color: INKU.magenta, border: 'none',
              padding: '10px 22px', borderRadius: 8, cursor: 'pointer',
              fontFamily: "-apple-system, BlinkMacSystemFont, 'Segoe UI', Helvetica, Arial, sans-serif", fontWeight: 700, fontSize: 14,
              display: 'inline-flex', alignItems: 'center', gap: 8,
            }}>Je m'inscris <Icon name="arrow-right" size={14} color={INKU.magenta} /></button>
          </form>
          <div style={{ marginTop: 16, fontSize: 12, color: 'rgba(255,255,255,0.7)', fontFamily: "-apple-system, BlinkMacSystemFont, 'Segoe UI', Helvetica, Arial, sans-serif" }}>
        ...
        <ul style={{ listStyle: 'none', padding: 0, margin: 0, display: 'flex', flexDirection: 'column', gap: 12 }}>
          {[
            ...
            <li key={i} style={{ display: 'flex', alignItems: 'center', gap: 12, fontFamily: "-apple-system, BlinkMacSystemFont, 'Segoe UI', Helvetica, Arial, sans-serif", fontSize: 15, color: '#fff' }}>
              <span style={{
    ...
    <footer style={{
      ...
      <div style={{ maxWidth: 1280, margin: '0 auto', padding: '0 32px' }}>
        <div style={{ display: 'grid', gridTemplateColumns: '1.2fr 1fr 1fr 1.2fr', gap: 48, marginBottom: 48 }}>
          <div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 16 }}>
              <span style={{ width: 32, height: 32, borderRadius: 7, background: INKU.magenta, display: 'inline-block' }} />
              <span style={{ fontFamily: "-apple-system, BlinkMacSystemFont, 'Segoe UI', Helvetica, Arial, sans-serif", fontWeight: 800, fontSize: 22, letterSpacing: '0.08em', color: '#fff' }}>INKU</span>
            </div>
            <p style={{ fontFamily: "-apple-system, BlinkMacSystemFont, 'Segoe UI', Helvetica, Arial, sans-serif", fontSize: 14, lineHeight: 1.6, maxWidth: 280 }}>
          ...
          <div>
            <div style={{ fontFamily: "-apple-system, BlinkMacSystemFont, 'Segoe UI', Helvetica, Arial, sans-serif", fontWeight: 700, fontSize: 13, color: '#fff', textTransform: 'uppercase', letterSpacing: '0.12em', marginBottom: 16 }}>Newsletter</div>
            <p style={{ fontFamily: "-apple-system, BlinkMacSystemFont, 'Segoe UI', Helvetica, Arial, sans-serif", fontSize: 13, lineHeight: 1.5, marginBottom: 14 }}>Le récap du dimanche. Un mail, c'est tout.</p>
        ...
        <div style={{
          ...
          fontFamily: "-apple-system, BlinkMacSystemFont, 'Segoe UI', Helvetica, Arial, sans-serif", fontSize: 12, color: 'rgba(255,255,255,0.5)',
        }}>
...
function FooterCol({ title, items }) {
  return (
    <div>
      <div style={{ fontFamily: "-apple-system, BlinkMacSystemFont, 'Segoe UI', Helvetica, Arial, sans-serif", fontWeight: 700, fontSize: 13, color: '#fff', textTransform: 'uppercase', letterSpacing: '0.12em', marginBottom: 16 }}>{title}</div>
      <ul style={{ listStyle: 'none', padding: 0, margin: 0, display: 'flex', flexDirection: 'column', gap: 10 }}>
        {items.map((it, i) => (
          <li key={i}><a href="#" onClick={(e)=>{e.preventDefault(); it.onClick && it.onClick();}} style={{ color: 'rgba(255,255,255,0.7)', textDecoration: 'none', fontFamily: "-apple-system, BlinkMacSystemFont, 'Segoe UI', Helvetica, Arial, sans-serif", fontSize: 14 }}>{it.label}</a></li>
        ))}
      </ul>
    </div>
  );
